...
sat::solve(expression::op_ref o) {
  if(!o->is_cnf) {
    throw std::runtime_error("Can only solve CNF formulas!");
  }
  if(o->is_quant()) {
    throw std::runtime_error("Cannot solve a QBF with a pure SAT solver!");
  }

  char* command = strdup(solver_path_.c_str());
  char* args[solver_args_.size() + 1];
  args[solver_args_.size()] = NULL;
  for(size_t i = 0; i < solver_args_.size(); ++i) {
    args[i] = strdup(solver_args_[i].c_str());
  }

  xcc_sat_solver solver;
  memset(&solver, 0, sizeof(solver));
  xcc_sat_solver_init(&solver, 0, 0, command, args, environ);
