...
#define NEXT_OR_RETURN(TGT_BOOL)                   \
  do {                                             \
    auto next_res = this->next();                  \
    if(result* r = std::get_if<result>(&next_res)) \
      return *r;                                   \
    TGT_BOOL = std::get<bool>(next_res);           \
  } while(false)

#define NEXT_OR_RETURN_                            \
  do {                                             \
    auto next_res = this->next();                  \
    if(result* r = std::get_if<result>(&next_res)) \
      return *r;                                   \
...
#define NEXT_TRUE_OR_RETURN_                       \
  do {                                             \
    auto next_res = this->next(true);              \
    if(result* r = std::get_if<result>(&next_res)) \
      return *r;                                   \
  } while(false)
...
boole::next(bool lispmode) {
  if(scanner_in_error_state_)
    return false;

  auto scanner = [this, lispmode]() -> std::variant<bool, result> {
    cur_ = std::move(next_);
    next_.ident.clear();
    ...
    for(;;) {
      ...
      if(colon_) {
        if(c_ != '(') {
          return error("Colon must be followed by an opening paren!");
        }
        type = token::SubstitutingLPar;
        c_processed_ = true;
        colon_ = false;
        return true;
      }
      ...
      switch(c_) {
        ...
        case ':':
          if(colon_) {
            return error("Cannot have a colon followed by a colon!");
          }
          colon_ = true;
          c_processed_ = true;
          continue;
      ...
      if(c_ & 0b1000'0000) {
        ...
        switch(c_) {
          ...
          case '%':
            ...
            continue;
          default:
            // Lisp execution means that everything that cannot be understood
            // is an ident.
            type = token::Ident;
        }
