...
// Taken from StackOverflow: https://stackoverflow.com/a/18899027.
namespace string_concat_detail {
  ...
  string_size(String&& s) {
    ...
    using string_t =
      typename std::conditional<std::is_array<noref_t>::value,
                                noref_t,
  ...
  struct concatenate_impl<String, Rest...> {
    ...
    static void concatenate(std::string& result, String&& s, Rest&&... rest) {
      result += s;
      concatenate_impl<Rest...>::concatenate(result,
...
concat(Strings&&... strings) {
  std::string result;
  result.reserve(string_concat_detail::concatenate_impl<Strings...>::size(
    std::forward<Strings>(strings)...));
  string_concat_detail::concatenate_impl<Strings...>::concatenate(
